...
def gen_plot(df,p,speedup,pkl_name,func_name):
    ...
    p = Path.cwd()
    if speedup:
        plt.savefig(f"{p.parent.parent}\Figures\MPI\MPI_{func_name}_{pkl_name}_speedup.png")
    else:
        plt.savefig(f"{p.parent.parent}\Figures\MPI\MPI_{func_name}_{pkl_name}_norm.png")
